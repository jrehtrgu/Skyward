...
async function init() {
    ...
    // Ocultar HUD normal inicialmente en VR
    updateHUDVisibility();
}
...
function setupVRControllers() {
    ...
    // Rayos para los controladores (visual)
...
// ==================== CREAR HUD EN PLANOS 3D (SIN PLANOS FÍSICOS, SOLO SPRITES) ====================
function create3DHUD() {
    // ELIMINAMOS LOS PLANOS FÍSICOS - solo creamos los elementos de texto
    // pero mantenemos las mismas posiciones y rotaciones que los planos originales
    createHUDTextElements();
}

// ==================== CREAR ELEMENTOS DE TEXTO (CON POSICIONES FIJAS COMO LOS PLANOS ORIGINALES) ====================
function createHUDTextElements() {
    // Función para crear sprite de texto con estilo CSS
    function createTextSprite(text, position, rotation, scale = 1.0, isRadar = false, panelType = 'left') {
        ...
        context.fillStyle = 'rgba(0, 10, 20, 0.85)';
        context.fillRect(0, 0, textureCanvas.width, textureCanvas.height);

        // Borde exacto al CSS
        context.strokeStyle = 'rgba(0, 255, 255, 0.6)';
        context.lineWidth = 2;
        context.strokeRect(1, 1, textureCanvas.width-2, textureCanvas.height-2);

        // Sombra exacta al CSS
        ...
        if (isRadar) {
            ...
            // Contenido del panel - estilo CSS
            context.font = '14px Orbitron';
            context.fillStyle = '#7ff';
            ...
            // Barra de escudo solo para el panel derecho
        ...
        // Ajustar escalas para mejor visibilidad
        if (isRadar) {
    ...
    // ==================== POSICIONES Y ROTACIONES FIJAS ====================


    // Panel 1: Información de Combate - IZQUIERDA SUPERIOR
    const panel1Pos = new THREE.Vector3(-4, 1.2, -2.5);
    const panel1Rot = new THREE.Euler(-Math.PI / 12, Math.PI / 9, 0); // 15° hacia abajo, 20° hacia derecha
    ...
    // Panel 2: Estado de la Nave - DERECHA SUPERIOR
    const panel2Pos = new THREE.Vector3(4, 1.2, -2.5);
    const panel2Rot = new THREE.Euler(-Math.PI / 12, -Math.PI / 9, 0); // 15° hacia abajo, 20° hacia izquierda
    ...
    // Panel 3: Radar - DERECHA INFERIOR 
    const panel3Pos = new THREE.Vector3(0, -0.8, 0);
    ...
    // Añadir a la nave para que se mueven con ella
    ...
    hudElements = {
        panel1: text1, // Información de combate (izquierda)
        panel2: text2, // Estado de la nave (derecha)
        panel3: text3  // Radar (derecha inferior)
    };
}
...
function drawRadarOnCanvas(context, width, height) {
    ...
    // Flecha del jugador (apuntando siempre hacia arriba en el radar)
    context.fillStyle = '#0ff';
    context.beginPath();
    ...
    // Barrido del radar (animación estática)
...
function updateHUDTexture(textElement, text, isRadar = false, panelType = 'left') {
    ...
    if (isRadar) {
        ...
        if (panelType === 'right' && text.includes('Escudo:')) {
            ...
            // Relleno de la barra con gradiente según el valor
...
function drawRadarWithEnemies(context, width, height) {
    ...
    // Flecha del jugador (rotada según la orientación de la nave)
    ...
    enemies.forEach(enemy => {
        ...
        if (distance < 200) { // Solo mostrar enemigos dentro del rango del radar
            ...
            switch(enemy.userData.type) {
                ...
                case 'heavy': enemyColor = '#8844ff'; break;
...
// ==================== CREAR NAVE JUGADOR (MANTENER MODO NORMAL SIN CAMBIOS) ====================
function createSpaceship() {
    ...
    // En VR, la cámara no está fija en la nave
    ...
    // ==================== CREAR HUD 3D (SIN PLANOS FÍSICOS) ====================
    create3DHUD();
}
...
async function preloadEnemyModels() {
    const loadPromises = Object.entries(ENEMY_TYPES).map(([type, config]) => {
        return new Promise((resolve) => {
            if (!config.model) {
                console.warn(`⚠️ No hay modelo definido para ${type}`);
                resolve();
                return;
            }
...
function createEnemy(type) {
    ...
    if (LOADED_MODELS[type]) {
        ...
        console.error(`❌ Modelo no precargado para ${type}`);
        return null;
    }
...
function enemyShoot(enemy) {
    ...
    if (enemyShootSound && !enemyShootSound.isPlaying) {
...
function gameOver() {
    ...
    document.getElementById('survival-time').textContent = Math.round(gameTime) + 's';
...
function updatePlayerArrow() {
    ...
    const rotationY = spaceship.rotation.y;
    const degrees = THREE.MathUtils.radToDeg(rotationY);
    playerArrow.style.transform = `translate(-50%, -50%) rotate(${degrees}deg)`;
}
...
function updateRadar() {
    ...
    document.querySelectorAll('.radar-blip').forEach(blip => blip.remove());
    ...
    enemies.forEach(enemy => {
        ...
        if (distance < 200) {
            ...
            const relativeX = (enemyPos.x - spaceship.position.x) / 200 * 50;
            const relativeZ = (enemyPos.z - spaceship.position.z) / 200 * 50;
            
            blip.style.left = `calc(50% + ${relativeX}px)`;
            blip.style.top = `calc(50% + ${-relativeZ}px)`;
            
            const size = Math.max(6, 10 - (distance / 200 * 4));
            blip.style.width = `${size}px`;
            blip.style.height = `${size}px`;
            
            if (enemy.userData.type === 'scout') {
                blip.style.background = '#00ff88';
            } else if (enemy.userData.type === 'fighter') {
                blip.style.background = '#ffaa00';
            } else {
                blip.style.background = '#8844ff';
            }
    ...
    const closestEnemy = document.getElementById('closest-enemy');
    if (closestEnemy) {
        closestEnemy.textContent = closestDistance < Infinity ? `${Math.round(closestDistance)}m` : '-';
    }
}
...
function updateHUDVisibility() {
    ...
    if (hud) hud.style.display = isVR ? 'none' : 'block';
    if (controls) controls.style.display = isVR ? 'none' : 'block';
}

function updateHUD() {
    const speed = spaceshipBody ? spaceshipBody.velocity.length() : velocity.length();
    
    // Actualizar HUD normal (solo modo no-VR)
    if (!renderer.xr.isPresenting) {
        ...
        if (shieldEl) shieldEl.textContent = `${Math.round(shield)}%`;
        if (enemiesEl) enemiesEl.textContent = `${enemies.length}/${ENEMY_LIMIT}`;
        if (scoreEl) scoreEl.textContent = score;
        if (timeEl) timeEl.textContent = `${Math.round(gameTime)}s`;
...
function update3DHUD() {
    ...
    updateHUDTexture(
        hudElements.panel2,
        `Velocidad: ${speed.toFixed(1)} km/s\nEscudo: ${Math.round(shield)}%\nMás Cercano: ${closestDistance < Infinity ? Math.round(closestDistance) + 'm' : '-'}`,
        false,
        'right'
    );

    // Actualizar panel 3: Radar (derecha inferior)
    updateHUDTexture(
        hudElements.panel3,
        '',
        true
...
unlockButton.addEventListener('click', async () => {
    try {
        await ambientSound.context.resume();
